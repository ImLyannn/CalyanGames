...
const CharacterData = {
  Mage: {
    ...
    basic:{name:'Arcane Bolt', icon:'✨', mult:0.9, isMagic:true, fx:{type:'bolt', color:0xb98aff}},
    passive:{name:'Mana Flow', icon:'🔷', desc:'Setiap 4 Basic Attack, kembalikan 8 Mana.'},
    skill1:{name:'Frozen Spike', icon:'❄️', mult:1.6, isMagic:true, manaCost:25, cooldown:4, effect:{type:'stun', duration:1.0}, fx:{type:'ice', color:0xbfe8ff}, desc:'Damage + root musuh 1 detik.'},
    skill2:{name:'Fire Blast', icon:'🔥', mult:1.5, isMagic:true, manaCost:35, cooldown:6, aoe:true, effect:{type:'dot', dps:9, duration:3}, fx:{type:'fire', color:0xff7a3f}, desc:'Ledakan area + Burn 3 detik.'},
    ...
    ultimate:{name:'Meteor Tornado', icon:'🌪️', mult:3.2, isMagic:true, manaCost:90, cooldown:25, aoe:true, effect:{type:'stun', duration:1.5}, fx:{type:'tornado', color:0x8a5cff}, desc:'Damage besar area + Stun 1.5 detik.'}
  },
  Archer: {
    ...
    basic:{name:'Quick Shot', icon:'🏹', mult:1.0, isMagic:false, fx:{type:'arrow', color:0x4fd68c}},
    passive:{name:'Hawk Eye', icon:'🦅', desc:'+8% Crit Rate selama 3 detik setelah pakai skill.'},
    skill1:{name:'Multi Shot', icon:'🎯', mult:1.35, isMagic:false, manaCost:18, cooldown:3, fx:{type:'arrow', color:0x8fe8b0}, desc:'Beberapa anak panah sekaligus.'},
    skill2:{name:'Piercing Arrow', icon:'🏹', mult:1.9, isMagic:false, manaCost:28, cooldown:5, defShred:0.3, fx:{type:'arrow', color:0xfff07a}, desc:'Damage tinggi, abaikan 30% Defense.'},
    skill3:{name:'Explosive Trap', icon:'💣', mult:1.4, isMagic:false, manaCost:24, cooldown:7, aoe:true, effect:{type:'slow', value:0.4, duration:2}, fx:{type:'shockwave', color:0xffb84f}, desc:'AoE + Slow 40% selama 2 detik.'},
    ultimate:{name:'Rain of Arrows', icon:'🌧️', mult:3.4, isMagic:false, manaCost:85, cooldown:24, aoe:true, fx:{type:'shockwave', color:0x4fd68c}, desc:'Hujan panah damage besar ke area.'}
  },
  Assassin: {
...
class GameApp{
  ...
  initPlayer(){
    ...
    this.player = {
      ...
      critRate:c.critRate, critDmg:c.critDmg, moveSpeed:c.moveSpeed,
  ...
  recalcStats(){
    const p = this.player;
    let hpPct=0, atkPct=0, magicPct=0, defPct=0;
    Object.values(p.equippedArtifacts).forEach(a=>{
      if(!a) return;
      if(a.statType==='hp') hpPct+=a.pct;
      else if(a.statType==='patk') atkPct+=a.pct;
      else if(a.statType==='magic') magicPct+=a.pct;
      else if(a.statType==='defense') defPct+=a.pct;
    });
  ...
  wireArtifactPanel(){
    document.querySelectorAll('#station-panel-body .art-slot.filled').forEach(el=>{
      el.addEventListener('click', ()=>{ this.unequipArtifact(el.dataset.slot); this.openStationPanel('artifact'); });
    });
    document.querySelectorAll('#station-panel-body .art-pool-item').forEach(el=>{
      el.addEventListener('click', ()=>{
        const art = this.player.artifacts.find(a=>a.id===el.dataset.id);
  ...
  initInput(){
    ...
    const canvas = document.getElementById('game-canvas');
    canvas.addEventListener('mousedown', e=>{ this.mouse.down=true; this.mouse.lastX=e.clientX; this.mouse.lastY=e.clientY; if(this.stageActive) this.tryAttack(); });
    ...
    document.getElementById('slot-dodge').addEventListener('click', ()=> { if(this.stageActive) this.tryDodge(); });
    ...
    document.getElementById('mbtn-dodge').addEventListener('touchstart', e=>{e.preventDefault(); if(this.stageActive) this.tryDodge();});
  ...
  spawnFX(fx, fromPos, toPos){
    ...
    switch(fx.type){
      ...
      case 'smoke':
        ...
        break;
      case 'shockwave':
        mesh = new THREE.Mesh(new THREE.RingGeometry(0.3,0.5,24), new THREE.MeshBasicMaterial({color, transparent:true, opacity:0.85, side:THREE.DoubleSide}));
        mesh.rotation.x=-Math.PI/2; mesh.position.copy(toP); mesh.position.y=0.06;
        life=0.35; kind='burst'; scaleFrom=1; scaleTo=3; baseOpacity=0.85;
        break;
      case 'tornado': {
  ...
  applySkillDamage(skillDef, isBasic, slotForLevel){
    ...
    if(effSkill.aoe){
      ...
      const range = effSkill.dash ? 6.5 : 2.3;
  ...
  trySkill(slot){
    ...
    p.mana -= s.manaCost; p.cooldowns[slot] = s.cooldown;
  ...
  tryDodge(){
    ...
    this.player.mesh.position.addScaledVector(dir, -2.0);
  }
  ...
  updateHUD(){
    ...
    this.updateCooldownVisual('slot-skill1', p.cooldowns.skill1, this.cdata.skill1.cooldown);
    this.updateCooldownVisual('slot-skill2', p.cooldowns.skill2, this.cdata.skill2.cooldown);
    this.updateCooldownVisual('slot-skill3', p.cooldowns.skill3, this.cdata.skill3.cooldown);
    this.updateCooldownVisual('slot-ultimate', p.cooldowns.ultimate, this.cdata.ultimate.cooldown);
  ...
  update(dt){
    ...
    const canMove = (this.stageActive || this.inLobby) && !this.panelOpen;
